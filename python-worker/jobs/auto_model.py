...
def handle_auto_model(job_id: str, org_id: str, object_id: str, logs: dict):
    ...
    try:
        ...
        update_progress(job_id, 80, {'status': 'creating_initial_runs'})
        
        # STEP 5: Create initial model runs (Baseline, Optimistic, Conservative)
        scenarios = [
            {'type': 'baseline', 'name': 'Baseline', 'revenue_boost': 1.0, 'cost_boost': 1.0},
            {'type': 'scenario', 'name': 'Optimistic', 'scenarioType': 'optimistic', 'revenue_boost': 1.25, 'cost_boost': 0.9},
            {'type': 'scenario', 'name': 'Conservative', 'scenarioType': 'conservative', 'revenue_boost': 0.75, 'cost_boost': 1.15},
        ]
        
        created_runs = []
        
        for sc in scenarios:
            # Create Run-specific params with overrides
            run_params = {
                **params,
                'autoGenerated': True,
            ...
            if sc['type'] == 'scenario':
                run_params['scenarioType'] = sc['scenarioType']
                # Apply logical multipliers to growth for scenarios
                rev_growth = assumptions.get('revenue', {}).get('revenueGrowth', 0.10)
                exp_growth = assumptions.get('costs', {}).get('expenseGrowth', 0.05)
                
                run_params['overrides'] = {
                    'revenue': {'growth': float(rev_growth * sc['revenue_boost'])},
                    'costs': {'growth': float(exp_growth * sc['cost_boost'])}
                }
            ...
            created_runs.append({'id': run_id, 'type': sc['type'], 'params': run_params})

        # STEP 6: Create jobs for all runs
        for run_info in created_runs:
            job_params = {
                'modelRunId': run_info['id'],
                'modelId': model_id,
                'runType': run_info['type'],
                'paramsJson': run_info['params']
            }
            ...
            job_logs = [
                {
                    ...
                    'level': 'info',
                    'msg': f"Job created via auto_model for {run_info['type']} run",
                    'meta': {
                        'jobType': 'model_run',
                        'params': job_params
                    }
        ...
        update_progress(job_id, 100, {
            'status': 'completed',
            'modelRunIds': [r['id'] for r in created_runs],
            'assumptionsGenerated': True,
        })
        
        conn.commit()
        
        logger.info(f"✅ Auto-model completed for model {model_id}, created {len(created_runs)} runs")
